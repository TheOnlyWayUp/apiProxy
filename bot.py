...
baseUrl = "https://uhhimactuallyoutheretouhhgetacupcake.mcson.top/mod/"
...
async def griefReq(ctx):
    await ctx.send(content="<@&919324310378790992>", embed=discord.Embed(title="Grief request", description=f"Sent by {ctx.author.mention}.", color=0x992D22))
...
async def illegal(ctx):
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"{baseUrl}returnIllegitimateHwids",
        ...
        async with session.get(
            ...
            for key in text.keys():
                if key != 'list':
                    ...
                    em.add_field(name=key, value=f'Owner - {text[key]["id"]} - {mention}.\nIPs - {", ".join(text[key]["ip"])}')
            
    await ctx.send(embed=em)

@bot.command(help="Change the version.")
@commands.has_any_role(919327476742750208, 924233487643451402)
async def changeVersion(ctx, version:str, downloadUrl:str, authorisedBy:discord.Member):
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"{baseUrl}changeVersion",
            headers={"Authorisation": "mcStalkerBased", 'version': version, 'downloadUrl': downloadUrl, 'authorisedBy': str(authorisedBy.id)},
        ) as resp:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            try:
                ...
                mention = "Not a Discord Member."
            em = discord.Embed(title=f"Version {data['versionString']}", description=f"Download URL - {data['downloadUrl']}\nAuthorised By - {data['authorisedBy']} - {mention}")
            await ctx.send(embed=em)
            return
